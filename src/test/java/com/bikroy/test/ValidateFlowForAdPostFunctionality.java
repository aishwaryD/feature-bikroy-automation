...
public class ValidateFlowForAdPostFunctionality extends BrowserApp {
	...
	public void tc_ValidateFlowForAdPostFunctionalityWithLogin() throws Exception {
		...
		String textMessage = this.propReader.readPropertyFromFile("AdPost_Success_Msg", "testData");
		this.msgsPage = this.msgsPage.verifyDisplayedMessageText(textMessage, "", TextComparators.contains);
		ValidateFlowForAdPostFunctionality.logger.info("Verify Text: Your ad was successfully submitted for review");

		// Click On My Ads Link Available On Ad Details Page
		this.myAddsPage = this.addDetailsPage.clickMyAdsLink();
		ValidateFlowForAdPostFunctionality.logger.info("Click On My Ads Link Available On Ad Details Page");

		// Validate Ads Under Review Count
		this.myAddsPage = this.myAddsPage.validateAdsUnderReviewCount();
		ValidateFlowForAdPostFunctionality.logger.info("Validate Ads Under Review Count");

		// Validate Posted Item Content
		String getItemTitle = this.propReader.readTestData("TC_1.1_ITEM_TITLE");
		this.myAddsPage = this.myAddsPage.validateItemContent(getItemTitle);
		ValidateFlowForAdPostFunctionality.logger.info("Validate Posted Item Content");
	...
	@Test(description = "Test Case 1.2: Validate users are able to post their advertisements on Bikroy "
			+ "web portal Without SignUp/Login.", dependsOnMethods = { "tc_ValidateFlowForAdPostFunctionalityWithLogin" })
	public void tc_ValidateFlowForAdPostFunctionalityWithOutSignUpLogin() throws Exception {
		...
		// Enter positive item details to post an advertisement
		String itemTitle = this.addDetailsPage.enterItemDetailsToPostAd(ProvideItemDetails.NOLOGGEDINUSER, "itemDetailsToPost", "idtp_1.0");
		this.propReader.updatePropertyInFile("TC_1.2_ITEM_TITLE", itemTitle, "testData");
		ValidateFlowForAdPostFunctionality.logger.info("Enter positive item details to post an advertisement");
